function init(){ensureGoogleReady()}
...
function ensureGoogleReady(){const start=Date.now();const maxWaitMs=15000;const tick=()=>{if(window.google&&window.google.accounts&&window.google.accounts.id){setupGsi();return}if(Date.now()-start>maxWaitMs){setStatus("Google auth failed to load. Check network or ad blockers.");return}setStatus("Loading Google authentication...");setTimeout(tick,300)};tick()}
function setupGsi(){const b=document.getElementById("gsi-button");google.accounts.id.initialize({client_id:window.CONFIG.CLIENT_ID,callback:onCredential,auto_select:false});google.accounts.id.renderButton(b,{theme:"filled_black",size:"large",width:320});setStatus("")}
